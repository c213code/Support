...
- **Главная страница** (`/`) — рабочий стол на любую дату (по умолчанию
  ...
  группам: ручная сортировка (стрелки), редактирование и удаление, живой
  предпросмотр готового текста репорта и кнопка "Скопировать".
...
- **Входящие** (`/inbox`) — рабочее место для разбора дня, с переключателем
  **Сообщения / Доска**:
  - **Сообщения** (по умолчанию) — лента сообщений из Telegram
    (`TelegramMessage`), которые пришли через вебхук бота
    (`/api/telegram/webhook`), с фильтром по группе (пилюли "Все" / название
    группы / "Без группы"). Каждое сообщение можно превратить в тикет (одной
    кнопкой, с автоподстановкой описания и ссылки) или скрыть. Привязка
    chat_id → группа запоминается в `GroupPreset.chatId` после первого
    ручного выбора.
  - **Доска** — канбан на весь экран, в 3 колонки: "Отправлено" / "В работе
    ...
    все новые тикеты стартуют в "Отправлено". Если за день остались тикеты
    со статусом `PENDING` (старый дефолт до перехода на `SENT`, то есть
    статус никто явно не выставлял) — появляется кнопка **"Пендинг без
    статуса → Отправлено"**, разово переносящая их в нужную колонку
