...
3. В **Settings → Environment Variables** добавь:
   ...
   - `AGENT_DEZHURNY_PASSWORD` — общий пароль для входа под именем
     "Дежурный" (для остальных саппортов, у которых нет отдельного аккаунта)
   ...
   - `OWN_AGENT_TELEGRAM_IDS` — Telegram id Ероша и Алпы через запятую (см.
     ниже, раздел "Свои сообщения не попадают во Входящие")
