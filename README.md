...
- **Тикеты** (`Issue`) привязаны к дате репорта (`reportDate`, формат
  ...
  и таймстемпы — это и есть базовое логирование. Автор проставляется
  сервером по сессии (кто залогинен), а не выбирается вручную в форме.
...
- **Главная страница** (`/`) — рабочий стол на любую дату (по умолчанию
  ...
  готового текста и кнопка "Скопировать". Есть переключатель **Доска / Список**:
  - **Доска** (по умолчанию) — канбан в 3 колонки: "Отправлено", "В работе /
    Пендинг" (статусы `IN_PROGRESS` и `PENDING` объединены — это один рабочий
    процесс, переключаются парой кнопок внутри карточки), "Решено". Тикет
    перетаскивается между колонками мышкой — статус меняется сразу.
  - **Список** — прежний вид по группам, с ручной сортировкой (стрелки),
    редактированием и удалением тикетов.
...
- **Входящие** (`/inbox`) — лента сообщений из Telegram (`TelegramMessage`),
  которые пришли через вебхук бота (`/api/telegram/webhook`). Сверху — фильтр
  по группе (пилюли "Все" / название группы / "Без группы"), чтобы быстро
  смотреть сообщения только из одного чата. Каждое сообщение можно превратить
  в тикет (одной кнопкой, с автоподстановкой описания и ссылки) или скрыть.
  Привязка chat_id → группа запоминается в `GroupPreset.chatId` после первого
  ручного выбора.
