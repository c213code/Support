...
// Выбор обращения куратором, когда он написал боту в личку без ответа
// стрелкой и непонятно, о каком из его обращений речь (см.
// PendingCuratorMessage). Формат: "sp:<submissionId>" либо "sp:x" — это
// новое обращение, отправить его формой.
export const SUBMISSION_PICK_PREFIX = "sp:";
...
// «Жаңадан бастау» под черновиком пересылки (см. lib/forwardDraft.ts):
// куратор начинает новый запрос, не дожидаясь, пока прежний черновик
// протухнет сам. Без аргументов — черновик у человека всегда один.
export const FORWARD_RESET = "fwd_reset";
