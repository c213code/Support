...
// только сузить лишние "пришлите почту", а не решать всё заново.
//
// Примеры даны прямо в промпте, а не few-shot диалогом, как в
// rewriteTicketDescriptionWithAI — задача бинарная (да/нет), и пары
// "сообщение → правильный ответ" внутри системного промпта хватает, чтобы
// модель не путала "нет явных данных в тексте" с "нужны данные, чтобы
// решить".
const ASK_HINT_SYSTEM_PROMPT = `Ты помогаешь боту поддержки JUZ40 решить: в автоответе на входящее обращение нужно ли попросить почту ученика или ссылку на урок/страницу.

Проси, если без этого проблему нельзя проверить или найти — это обращение про КОНКРЕТНОГО ученика или конкретную страницу: не заходит в кабинет, не открывается тест/урок, ошибка при входе, ссылка не работает.

Примеры, где просить НУЖНО (ask: true):
- "Мәселе: АА кабинетіне тіркеуде шақырту жіберіп, берілген логин парольді жазсақ қате деп кірмейді" — не заходит с конкретными данными, нужно проверить у кого.
- "Тест по математике не открывается, ошибка 404" — нужна ссылка/почта, чтобы найти именно этот тест у ученика.

Не проси, если вопрос общий и ответ на него не зависит от конкретного ученика: график/время работы платформы, как в целом устроен процесс, организационный вопрос, благодарность, вопрос коллеги по работе.

Примеры, где просить НЕ нужно (ask: false):
- "Платформада жұмыс жасау уақыты қандай?" (во сколько работает платформа) — общий вопрос про график, не про конкретного ученика.
- "Тексеру үшін оқушының почтасын не сілтемені жібере аласыз ба?" уже была задана и на неё ответили "рахмет" — это не новый повод спрашивать снова.

Ответь строго JSON без пояснений: {"ask": true} или {"ask": false}.`;

// Возвращает null при недоступности модели/ключа — вызывающий код тогда
// остаётся на исходном regex-решении (не спрашивать не значит "просить
// нельзя", а "не удалось уточнить").
export async function shouldAskForIdentifier(text: string): Promise<boolean | null> {
  ...
  try {
    ...
    return typeof parsed?.ask === "boolean" ? parsed.ask : null;
  } catch {
    return null;
  }
}
