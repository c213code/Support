...
const FEW_SHOT_TURNS: Array<{ user: string; assistant: string }> = [
  ...
  // уточнения, которое ещё не содержит самой проблемы.
  {
...
// Добавляется к SYSTEM_PROMPT, когда сообщение — реплай на сообщение
// одного из НАШИХ агентов (не клиента), см. repliesToOwnAgent в вебхуке.
// Такие цепочки статистически чаще оказываются внутренним уточнением
// ("проверь, пожалуйста"/"а что там"), а не пересказом готовой жалобы
// клиента — обычные правила SKIP это не всегда ловят, потому что вопрос
// коллеги может содержать те же слова ("оқушы", "не істеп тұр"), что и
// настоящая жалоба, просто без самой жалобы внутри.
const REPLIES_TO_OWN_AGENT_ADDENDUM = `

ВАЖНО: это ответ на сообщение ОДНОГО ИЗ НАШИХ АГЕНТОВ поддержки, а не клиента — цитата сверху от нашего же сотрудника. Такие цепочки чаще всего внутренние: агент сам о чём-то спрашивает или уточняет, коллега уточняет в ответ — конкретной жалобы клиента там может не быть вообще, даже если упоминается ученик. Заводи тикет только если из переписки понятна КОНКРЕТНАЯ проблема конкретного ученика (что именно не работает/не так). Если это лишь вопросы друг другу без сформулированной проблемы ("что случилось", "проверьте, пожалуйста", "это нормально?") — SKIP.`;
        ...
        messages: [
          {
            ...
            content:
              SYSTEM_PROMPT +
              (repliesToOwnAgent ? REPLIES_TO_OWN_AGENT_ADDENDUM : "") +
