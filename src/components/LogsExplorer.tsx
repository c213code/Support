...
function ColumnHeader({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      <span className="grid h-4 w-4 shrink-0 place-items-center rounded bg-sky-100 text-[9px] font-bold text-sky-700">
        t
      </span>
      <span className="font-mono text-[11px] font-semibold text-slate-600">{children}</span>
    </span>
  );
}
          ...
          {result.hits.length === 0 ? (
            ...
            <div className="max-h-[70vh] overflow-auto">
              <table className="w-full min-w-[1120px] border-collapse text-left text-sm">
                <thead>
                  <tr>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Время</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Метод</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Статус</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>URI</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Кто</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Устройство</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 whitespace-nowrap border-b border-r border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Версия</ColumnHeader>
                    </th>
                    <th className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50 px-2.5 py-2">
                      <ColumnHeader>Сообщение</ColumnHeader>
                    </th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <Fragment key={key}>
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap border-r border-slate-100 px-2.5 py-1.5 text-slate-600">
                            {hit.method ?? "—"}
                          </td>
                          <td className="whitespace-nowrap border-r border-slate-100 px-2.5 py-1.5">
                            <span
                              className={`rounded px-1.5 py-0.5 font-semibold ${statusBadgeClass(hit.status)}`}
                            >
                              {hit.status ?? "—"}
                            </span>
                          </td>
                          <td className="max-w-xs truncate border-r border-slate-100 px-2.5 py-1.5 text-slate-700">
                            {hit.uri ?? "—"}
                          </td>
                          <td className="max-w-[160px] truncate border-r border-slate-100 px-2.5 py-1.5 text-slate-500">
                            {hit.username ?? "—"}
                          </td>
                          <td className="whitespace-nowrap border-r border-slate-100 px-2.5 py-1.5 text-slate-500">
